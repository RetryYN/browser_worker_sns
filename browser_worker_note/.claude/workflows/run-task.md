...
1. タスクYAMLの `skill` 指定を確認し、`.agent/skills/<skill>/SKILL.md` を Read で読み込む
2. SKILL.md 内の「必須参照リソース」セクションに記載されたリソースファイルを Read で読み込む
3. `reference` が指定されている場合、対応するリソースファイルも Read で読み込む
4. `config/personas.yaml` を Read で読み込む（committee_review の場合）
...
Claude が各ステップの出力（変更前の値、生成したテキスト、確認結果など）をテキストとして保持し、次のステップの `description` 内の `{{prev_result}}` を置換して解釈する。
...
3. 生成結果を `step_results` に格納して次のステップへ

### G-7. action: committee_review（組織的監査）

1. Step C（C-5）で読み込み済みの `personas.yaml` とスキルを参照する
...
3. **Phase 2: 下流修正 (Downstream)**
   ...
   - 例: Writer(Downstream, skill: copywriting) → copywriting スキルで文章のリズム・読みやすさを改善
4. **Phase 3: 最終承認 (Executor)**
   - `executor` ロール（通常 CEO）の personas.yaml **Executor** 基準で最終判断
   - 各部門の意見が対立した場合、企業のビジョン・ミッションに最も合致する選択をする
5. 最終成果物を `step_results` に格納する

### G-8. action: generate_image

> **注意**: Claude Code には画像生成ツールがない。
> 外部APIを使う場合は `Bash` で `curl` 等を実行する。
> この機能は現在未実装。実行時にユーザーに通知して代替手段を相談する。

### G-9. action: upload_image

1. `mcp__playwright__browser_file_upload` で画像ファイルをアップロードする
2. `mcp__playwright__browser_snapshot` でアップロード結果を確認する
