...
| `load_draft` | G-11 | `Read` + パース | `description` (パス・抽出指示) | note記事で検証済み |
| `load_handoff` | G-12 | `Read` + YAML パース | `handoff_path` | 未テスト |

**条件付き実行**: `when` / `when_not` フィールドで制御する
- `when`: 値が空文字・`false`・`0`・`none`・`null`・`skip` なら `skipped`
- `when_not`: 値が**空でない**場合に `skipped`（`when` の逆。排他的フォールバック用）
- 両方指定された場合は `when` が優先

**結果参照**: `{{prev_result}}`, `{{step_results[N]}}`, `{{step_results.<step_id>}}` で前ステップの出力を参照
- `{{step_results[N]}}`: インデックス参照（0始まり）。`skipped` ステップもカウントする
- `{{step_results.<step_id>}}`: 名前参照（推奨）。ステップに `id` フィールドがあればその値で参照可能。`when` スキップでインデックスがずれるリスクを回避できる

### タスク複雑度ティア

タスクYAMLに `tier` フィールドを追加することで、A-K ステップの実行範囲を制御できる。

| tier | 対象タスク | スキップするステップ | 例 |
|------|-----------|-------------------|-----|
| 0 (trivial) | テキスト・画像が準備済みの直接投稿 | B, C, D, J をスキップ | `x-post-publish` |
| 1 (standard) | 通常のタスク。全 A-K ステップ | なし（デフォルト） | `note-article-publish` |
...
ブラウザ操作前に `.playwright-mcp/` の古いログ・スクリーンショットを削除する。

```bash
# Playwright MCPのログ・スクショ
rm -f .playwright-mcp/*.log .playwright-mcp/*.jpeg .playwright-mcp/*.png
# リポジトリルートに散らばった作業用スクショ
rm -f *.png *.jpeg
```

※ 正規の画像は `knowledge/data/images/` と `knowledge/characters/` に保存されており影響しない。
※ `.gitignore` 済みだがディスクを圧迫するため毎回実行する。
...
8. 使える変数は `{{site}}`, `{{param_name}}`, `{{prev_result}}`, `{{step_results[N]}}`, `{{step_results.<step_id>}}`
9. ステップに `when` がある場合は、その値もテンプレート解決する。解決後の値が空文字、`false`, `0`, `none`, `null`, `skip` のいずれかなら、そのステップは `skipped` として記録し次へ進む。`when_not` がある場合は逆（値が空でなければ skipped）
...
各タスクステップの実行結果は `step_results` として蓄積する。後続ステップで `{{prev_result}}`, `{{step_results[N]}}`, `{{step_results.<step_id>}}` として参照可能。

```
step_results = []       # インデックス参照用（0始まり）
step_results_by_id = {} # 名前参照用（id フィールドがあるステップのみ）

ステップ1（id: gen_text）実行 → step_results[0] に格納 + step_results_by_id["gen_text"] にも格納
ステップ2実行 → {{prev_result}} は step_results[0]。{{step_results.gen_text}} も同じ値
ステップ3（when: skipped）→ step_results[1] = "skipped"。インデックスはカウントされる
ステップ4実行 → {{prev_result}} は step_results[1]（"skipped"）。{{step_results.gen_text}} は依然ステップ1の値
```

**推奨**: `when` / `when_not` で条件スキップがあるタスクでは、名前参照（`id` + `{{step_results.<step_id>}}`）を使う。インデックス参照はスキップでずれるため。

Claude は各ステップ実行前に、そのステップ配下のすべての文字列フィールドへテンプレート解決を適用する。

`when` がある場合は、解決後の値で実行可否を判定する。空文字、`false`, `0`, `none`, `null`, `skip` は偽として扱い、そのステップを `skipped` にする。`when_not` は逆判定（値が空でなければ skipped）。
...
### G-11. action: load_draft

下書きファースト原則で、校正済み下書きファイルから記事本文を読み込む。

1. `description` に記載されたパスのファイルを `Read` で読み込む
2. ステータスが `校正済み` であることを確認する（それ以外なら中断し理由を報告）
3. フロントマター（タイトル、C×P×T、ハッシュタグ、想定文字数）を抽出する
4. 画像マニフェストテーブルからファイルパスを抽出し、各ファイルの存在を確認する
5. 本文（LEAD / BODY / CTA）を抽出し、`step_results` に構造化して記録する

### G-12. action: load_handoff

リサーチハンドオフ YAML を自動読み込みする。

1. `handoff_path` で指定されたファイルを `Read` で読み込む
2. ファイル内の handoff YAML ブロック（```yaml ... ```）をパースする
3. 抽出したキー（theme, source_url, content_type, treatment, info_vectors 等）を `step_results` に記録する
4. 後続ステップで `{{step_results.<step_id>}}` として各フィールドを参照可能にする
