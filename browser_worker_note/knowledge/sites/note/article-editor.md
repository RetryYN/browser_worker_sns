...
### 画像挿入
1. **空の段落**をクリック（⚠️ テキストがある段落では絶対にやらないこと）
2. 「メニューを開く」(+ボタン)をクリック
3. 「画像」をクリック
4. ファイルチューザーが開く → `browser_file_upload` で絶対パスを指定

⚠️ **テキスト分断注意**: テキスト段落の途中にカーソルがある状態で画像を挿入すると、カーソル位置でテキストが2つの段落に分断される。必ず空の段落から操作すること。記事末尾に画像を追加する場合は、最後のテキスト段落の下にある空段落の+メニューを使う。
